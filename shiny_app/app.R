...
ui <- fluidPage(
  ...
  fluidRow(
    column(4, selectizeInput('searchselect', label= 'search terms',choices = NULL,multiple=TRUE)),
    #  multiple = TRUE,selected = c("kapo zh","kantonspolizei zürich","steueramt zürich","private tax 2017","steuererklärung 2017","steuerrechner zürich","quellensteuer zürich","kantonales steueramt zürich","steuererklärung zürich","steuern zürich"), options= list(maxItems = 10))),
    column(4,  selectInput('domainselect', label='subdomain', choices=top_domain, selected = NULL, multiple = TRUE, selectize = TRUE))
  ),
  column(12,"By checking the checkbox below you can decompose the search terms into single words."),
  ...
  column(12,"The flows reveal how many impressions of a site a search term has generated (transpartent flow) as well as clicks (saturated flow)."),
...
server <- function(input, output,session) {
  ...
  updateSelectizeInput(session=session, 'searchselect', choices = searchterms, server = TRUE, selected=top10$Suchanfragen)
